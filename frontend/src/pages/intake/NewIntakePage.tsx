...
const toIntSafe = (v: any, fallback = 1) => {
...
const NewIntakePage: React.FC = () => {
  ...
  // ✅ ตัวเลขที่ใช้จริง
  const qtyNum = useMemo(() => toIntSafe(form.quantity || "1", 1), [form.quantity]);
  ...
  const validate = (): string | null => {
    ...
    // ✅ (แนะนำ) บังคับรูปอย่างน้อย 1 รูปของสินค้า + บัตร
    // ถ้าคุณไม่อยากบังคับ ให้คอมเมนต์ 2 บรรทัดนี้
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // ✅ ให้ตรง backend /api/intake (multer) ใช้ FormData
      ...
      fd.append("sourceType", "PURCHASE"); // ✅ ให้ตรง enum จริง
      ...
      // ✅ endpoint ที่ถูกต้องกับ backend intake.js ของคุณ
      await api.post("/api/intake", fd);

      alert("บันทึกการรับซื้อสินค้าเรียบร้อย");
      navigate("/app/inventory"); // หรือ /app/customers แล้วแต่ flow ที่คุณชอบ
    } catch (err: any) {
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              บันทึกการรับซื้อสินค้าเข้าสต๊อก + บันทึกรายจ่าย (ต้นทุนรวม) + รองรับอัปโหลดรูปหลักฐาน
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl bg-white p-6 shadow-sm">
          <div className="grid gap-6 md:grid-cols-2">
            {/* ผู้ขาย */}
            <section>
              <h2 className="mb-4 text-base font-semibold text-slate-800">1. ข้อมูลผู้ขาย (ลูกค้า)</h2>
              ...
              <div className="mb-4">
                <label className="mb-1 block text-xs font-medium text-slate-700">
                  รูปบัตรประชาชน (แนะนำให้อัปโหลด)
                </label>
                <div className="flex items-center gap-3">
                  <label className="flex h-24 w-24 cursor-pointer items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 text-xs text-slate-500 hover:bg-slate-100">
                    <span>+ เพิ่มรูป</span>
                    <input type="file" accept="image/*" className="hidden" onChange={handleFileChange("idcard")} />
                  </label>

                  {idCardPreview && (
                    <img src={idCardPreview} alt="ID card preview" className="h-24 w-32 rounded-lg object-cover shadow-sm" />
                  )}
                </div>
              </div>
            ...
            <section>
              ...
              <div className="space-y-3">
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-700">ชื่อสินค้า / รุ่น (Brand/Model)</label>
                  <input
                    name="brandModel"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-700">Serial No. / IMEI</label>
                    <input
                      name="serial"
                      value={form.serial}
                      onChange={handleChange}
                ...
                <div className="mt-2">
                  <label className="mb-1 block text-xs font-medium text-slate-700">รูปภาพสินค้า (อย่างน้อย 1 รูป)</label>
                  <div className="flex items-center gap-3">
                    <label className="flex h-24 w-24 cursor-pointer items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 text-xs text-slate-500 hover:bg-slate-100">
                      <span>+ เพิ่มรูป</span>
                      <input type="file" accept="image/*" multiple className="hidden" onChange={handleFileChange("product")} />
                    </label>

                    {productPreview && (
                      <img src={productPreview} alt="Product preview" className="h-24 w-32 rounded-lg object-cover shadow-sm" />
                    )}

                    {productFiles && productFiles.length > 1 && (
                      <div className="text-xs text-slate-500">เลือกแล้ว {productFiles.length} รูป</div>
                    )}
                  </div>
                </div>

                <div className="mt-3">
                  <label className="mb-1 block text-xs font-medium text-slate-700">รูปสัญญา / หลักฐานอื่น ๆ (ไม่บังคับ)</label>
                  <div className="flex items-center gap-3">
                    <label className="flex h-20 w-20 cursor-pointer items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 text-xs text-slate-500 hover:bg-slate-100">
                      <span>+ เพิ่มรูป</span>
                      <input type="file" accept="image/*" multiple className="hidden" onChange={handleFileChange("contract")} />
                    </label>

                    {contractPreview && (
                      <img src={contractPreview} alt="Contract preview" className="h-20 w-28 rounded-lg object-cover shadow-sm" />
                    )}

                    {contractFiles && contractFiles.length > 1 && (
                      <div className="text-xs text-slate-500">เลือกแล้ว {contractFiles.length} รูป</div>
                    )}
                  </div>
                </div>
              </div>
          ...
          <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-red-600">ราคาต่อชิ้น (Purchase / ชิ้น)</label>
                <input
                  name="unitPrice"
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-emerald-700">ราคาตั้งขายต่อชิ้น (Target)</label>
                <input
                  name="targetPrice"
              ...
              <div className="md:w-1/3">
                <div className="rounded-xl bg-white px-4 py-3 text-right shadow-inner">
                  ...
                  <div className="mt-1 text-[11px] text-slate-500">
                    * ต่อชิ้น: {money(unitNum)} บาท • จำนวน: {qtyNum} ชิ้น
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mt-3 text-[11px] text-slate-500">
            * ไฟล์นี้ส่งแบบ FormData ไป <b>/api/intake</b> (ตรงกับ backend intake.js) เพื่อให้ต้นทุน/คงเหลือ/รูปหลักฐานถูกต้อง
          </div>
        </form>
      </div>
    </div>
